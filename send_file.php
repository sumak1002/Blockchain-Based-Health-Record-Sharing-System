...
<center>
 ...
 <select class="form-control"  class="span7" name="email" required>
...
$Staff_Email=$row['Staff_Email'];
                    ...
                    echo "<option value='$Staff_Email'>$Staff_Email</option>";
                  }
                
                ?>
                  </select>

<input type="text" name="msg" class="span7" placeholder="Message"><br>
<input type="submit" class="btn btn-info" value="Send" style="width:150px; margin-left:-80px;">
</center>
</form>
</div>
...
$fileid=$_GET['id'];$sender_id=$_SESSION['userid'];
$chipertext=$receiver_private_key."_".$sender_public_key;
if(share_file($fileid,$sender_public_key,$receiver_private_key,$sender_id,$receiver_id)==1)
{
$stmt1 = $db->prepare("SELECT * FROM  file_sharing ORDER by id desc limit 1");
$stmt1->execute();
while($row1 = $stmt1->fetch(PDO::FETCH_ASSOC)) {
$lastid=$row1['id'];
}
update_chiper_text($lastid,$chipertext);
update_receiver_server($msg,$lastid,$receiver_id,$chipertext,$receiver_public_key,$receiver_private_key,$fileid);

echo "<script>alert('Data Sent Succesfully');</script>";
}
else
{
